...
| `connector_authorization` | V0004 | Auth | Per `(lea_id, partner)` authorization state. Partial unique index on `(lea_id, partner) WHERE revoked_at IS NULL` keeps the dashboard read honest. Carries no credential: EdLink owns the token, addressed by `leas.edlink_integration_id` (V0009); the `secret_ref` column was dropped in V0012. |
