...
| Background scheduler | **Azure Functions timer triggers** (poll cadence, daily reconciliation) | procrastinate, apscheduler, arq | Resolves Q-020 to the Azure-native scheduler now that the cloud is Azure. Timer triggers run on the shared Flex Consumption plan. procrastinate was the provisional non-Azure pick. Confirm at the day-one walkthrough (Q-009). | Recommended |
| Event bus transport | **Azure Service Bus** (ingest queue with sessions, `session_id=lea_id`; a topic for downstream fan-out) | Redis Streams, RabbitMQ, Kafka | Resolves Q-019 to the Azure-native transport. Sessions give per-LEA serialization; Standard tier carries sessions and duplicate detection. Redis Streams was the provisional pick. Confirm at the day-one walkthrough (Q-009). | Recommended |
...
| Observability / logging sink | **Azure Application Insights** via the Azure Monitor OpenTelemetry distro (`azure-monitor-opentelemetry`, one `configure_azure_monitor()` call) | Datadog, Honeycomb, self-hosted Loki | One distro exports traces, metrics, and logs. Sampling is off by default; alert on custom metrics, which are never sampled. `opencensus-ext-azure` is retired (Sept 2024), do not use it. | **Accepted** |
...
| Tracing | **azure-monitor-opentelemetry** distro (wraps the OTel SDK and Azure exporter) | raw opentelemetry-distro + azure-monitor-opentelemetry-exporter | One `configure_azure_monitor()` call wires traces, metrics, and logs together; emit W3C traceparent so a poll-to-canonical-write traces end-to-end | Recommended |
